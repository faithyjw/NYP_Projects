...
print("Unique values in Mileage Unit: ", df1["Mileage Unit"].unique())
...
print("Unique values in Max Power Unit: ", df1["Max Power Unit"].unique())
...
df1 = df1.dropna(subset=['No. of Seats', 'Mileage (kmpl)',
                         'Engine (cc)', 'Max Power (bhp)'])
#remove the row that contained "bhp")
df1= df1[~df1['Max Power (bhp)'].str.contains('bhp', na=False)]
...
plt.figure(figsize=(10, 6))
sns.regplot(x='No. of Seats', y='Sold Price ($)', data=df1, 
...
plt.show()
#drop column
df1 = df1.drop(columns= ["No. of Seats"])

#partioning dataframe
df_shuffled = df1.sample(frac=1, random_state=42).reset_index(drop=True)
split_index = int(0.8 * len(df_shuffled)) #calculate the split index
df80= df_shuffled[:split_index] # Split DataFrame
#export csv df to excel df
df80.to_excel('PAP_CarTraining.xlsx', sheet_name= "training_data", index=False)

#export csv df to excel df
df20 = df_shuffled[split_index:]
df20.to_excel('PAP_CarTest.xlsx', sheet_name= "test_data", index=False)